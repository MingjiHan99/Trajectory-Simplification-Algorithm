...
int main(int argc,char *argv[]){
    ...
    double tx,ty,tt;
    double averge_rate,temp_rate,average_second;
    double total_second = 0.0;

    for(int i = 0 ; i < size; i++){
        Trajectory<Point>* traj = new Trajectory<Point>;

        std::string file_name = "../dataset/taxi_map/A000" + std::to_string(i);
        ...
        total_second += (double)(end_time - start_time) / CLOCKS_PER_SEC;
    ...
    std::cout << "Average Running time " << (double) average_second << "s"<< std::endl;
